...
stageOutputs <- function(id, f1, selCrit, stage, year, bsp){
  ...
  return(tibble(cycle=year-stage, year=year, stage=stageName,
                ...
                genValMean=mean(gv(f1)), genValSD=sd(gv(f1)),
                evalAtSelMean=mean(selCrit, na.rm=T), evalAtSelSD=sd(selCrit, na.rm=T),
                accAtSel=cor(gv(f1), selCrit), gvOfBestCrit=gvOfBestCrit,
                highestGV=highestGV, nContribToPar=list(NA)))
}
